...
var defaultPeopleCountGetter = (req, res, next) => {
	if(req.params.startTime && req.params.endTime) {
		next(GETTER_WITH_START_END_TIME);
	} else {
		var startTime = new Date();
		startTime.setHours(0,0,0,0); //setting time to midnight 12AM
		var endTime = new Date();
		mongoDao.getPeopleCountFromEvents(startTime, endTime, peopleCount => {
...
var getterWithStartEndTime = (req, res, next) => {
	if(req.params.startTime && req.params.endTime) {
		mongoDao.getPeopleCountFromEvents(new Date(req.params.startTime), new Date(req.params.endTime), 
			peopleCount => {
				res.send(200,peopleCount);
				next(false);
			});
